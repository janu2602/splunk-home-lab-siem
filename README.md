...
**Splunk Server**: Ubuntu 24.04 VM running Splunk Enterprise as the "brain" for indexing and searching.
**Monitored Endpoint**: Windows 10 VM acting as the "sensor," equipped with Sysmon for high-fidelity logging.
**Log Transport**: Splunk Universal Forwarder (UF) installed on the Windows host to stream logs over port 9997.
**Network**: Both VMs communicate over a dedicated VirtualBox NAT Network (10.0.2.0/24) to ensure internal connectivity and internet access for updates.
...
**Skills Demonstrated**
**SIEM Deployment**: Installation and configuration of Splunk Enterprise on Linux.
**Log Ingestion Pipeline**: Setting up a receiving port and configuring a Universal Forwarder.
**Endpoint Telemetry**: Implementing Sysmon with the SwiftOnSecurity configuration to capture process creation and network connections.
**SPL Proficiency**: Writing Search Processing Language (SPL) queries to filter and aggregate security events.
**Dashboarding**: Designing a "Security Monitoring" dashboard for real-time visualization of threats.
...
**Key Searches (SPL)**
The following queries power the core panels of the Security Dashboard:
[Splunk SIEM Home Lab_ Key Searches (SPL).xlsx](https://github.com/user-attachments/files/25655815/Splunk.SIEM.Home.Lab_.Key.Searches.SPL.xlsx)
